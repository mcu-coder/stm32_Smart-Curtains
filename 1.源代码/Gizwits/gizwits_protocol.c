...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueCO != cur->valueCO)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagCO = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagCO)
    {
        if(dataPoints->valueCO >= CO_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueCO Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,CO_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueCO);
                bitFieldOffset += CO_LEN;
        }
    }
